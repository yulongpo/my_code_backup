...
public:
	...
	std::vector<band_info> detect(const char* data, const unsigned len) {
		set_data(data, len);
		return detect();
	}
	...
	template <typename T>
	void set_data(const T* data, const unsigned data_len) {
		data_origin = std::vector<float>(data, data + data_len);
		data_origin_tensor = torch::from_blob(data_origin.data(), (data_origin.size()));
	}
